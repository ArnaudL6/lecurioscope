...
export function showSystemNotif({title='Qu�te accomplie',xpGain=0,rank=null}){
  ...
  el.innerHTML=`
    <div class="sl-notif-inner" style="border-color:${r.color};box-shadow:${r.glow};">
      <div class="sl-notif-top"><span class="sl-notif-icon">�</span><span class="sl-notif-label">QU�TE ACCOMPLIE</span></div>
  ...
  el.innerHTML=`
    <div class="sl-levelup-inner">
      ...
      <div class="sl-levelup-content">
        <div class="sl-levelup-label">RANG SUP�RIEUR D�BLOQU�</div>
        ...
        <div class="sl-levelup-sub">F�licitations, Chasseur.</div>
      </div>
  ...
  bd.innerHTML=`
    <div class="sl-stats-panel" style="--rank-color:${rank.color};--rank-glow:${rank.glow};">
      ...
      <div class="sl-stats-header">
        <div class="sl-stats-title">FEN�TRE DE STATUT</div>
      ...
      <div class="sl-stats-grid">
        ${[
          ...
          {key:'FOR',label:'Force',val:FOR,desc:enigmaCorrect+' �nigmes r�solues',color:'#a855f7'},
        ].map(s=>`
...
export async function checkAndAwardBadges(badgeData){
  ...
  try{
    ...
    for(const badge of newOnes){
      ...
      showToast('<� Badge d�bloqu� : '+badge.icon+' '+badge.name+' !');
