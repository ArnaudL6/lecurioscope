...
export async function goEnigme(){
  ...
  show('screen-load');
  const lt=document.getElementById('load-title');if(lt)lt.textContent='Chargement de l\'�nigme&';
  try{
...
export async function pickEnigmeCat(){
  ...
  const lt=document.getElementById('load-title');if(lt)lt.textContent='G�n�ration de l\'�nigme&';
  show('screen-load');
  try{
    ...
    if(!res.ok){showToast('� Erreur serveur. R�essaie !');buildEnigmePick();show('screen-enigme-pick');return;}
    const{enigma,choice}=await res.json();
    if(!enigma){showToast('� G�n�ration �chou�e. R�essaie !');buildEnigmePick();show('screen-enigme-pick');return;}
    state.todayEnigme=enigma;state.todayEnigmeChoice=choice;
    showEnigme(true);
  }catch(e){console.error(e);showToast('� Erreur : '+(e?.message||'r�seau'));buildEnigmePick();show('screen-enigme-pick');}
}
...
export function showEnigme(typewrite){
  ...
  const chooserEl=document.getElementById('enigme-chooser');if(chooserEl)chooserEl.textContent=state.todayEnigmeChoice?.chooser||'Communaut�';
  // Difficulty badge
  const diffMap={easy:'Facile',medium:'Interm�diaire',hard:'Difficile'};
...
export function appendChatMsg(msg){
  const box=document.getElementById('chat-msgs');if(!box)return;
  // D�duplique par id pour �viter double-affichage
...
export async function initEnigmeRating(){
  ...
  if(allR&&allR.length){const avg=(allR.reduce((s,r)=>s+r.stars,0)/allR.length).toFixed(1);const el=document.getElementById('enigme-rating-avg');if(el)el.textContent='Moyenne : '+avg+' P ('+allR.length+')';}
}
...
export async function loadEnigmaCommentsFeed(){
  ...
  if(!comments||!comments.length){feed.innerHTML='<div class="comment-empty">Sois le premier � commenter !</div>';return;}
...
export function showSondageWIP(){
  ...
  document.getElementById('top-tab-anec')?.classList.remove('active');
  showToast('=� Sondage du jour  Bient�t disponible !');
  setTimeout(()=>{
    document.getElementById('top-tab-sondage')?.classList.remove('active');
