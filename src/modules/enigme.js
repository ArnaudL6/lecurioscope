...
export async function pickEnigmeCat(){
  ...
  try{
    ...
    const chooserId=state.currentUser?currentUser.id:null;
...
export function revealEnigmaAnswer(correct,userAnswer,serverAnswer){
  if(state.todayEnigme)localStorage.removeItem('enigma_att_'+todayEnigme.id);
...
export async function revealEnigmaWithoutAnswer(){
  ...
  let serverAnswer=null;
  try{const r=await fetch(EDGE_ENIGME,{method:'POST',headers:{'Authorization':'Bearer '+SB_ANON,'apikey':SB_ANON,'Content-Type':'application/json'},body:JSON.stringify({action:'check_answer',enigmaId:todayEnigme.id,userAnswer:''})});if(r.ok){const d=await r.json();serverAnswer=d.answer||null;}}catch(e){}
  if(state.currentUser){await sb.from('enigma_responses').upsert({user_id:currentUser.id,enigma_id:todayEnigme.id,date:todayEnigme.date,category:todayEnigme.category,response_text:'',is_correct:false},{onConflict:'user_id,enigma_id'});}
...
export function appendChatMsg(msg){
  ...
  const isMine=state.currentUser&&msg.user_id===currentUser.id;
...
export async function submitEnigmaRating(){
  ...
  await sb.from('enigma_ratings').upsert({user_id:currentUser.id,enigma_id:todayEnigme.id,stars:state.enigmeCurRating,comment},{onConflict:'user_id,enigma_id'});
...
export async function loadEnigmaCommentsFeed(){
  ...
  feed.innerHTML=comments.map(c=>{
    const isOwn=state.currentUser&&c.user_id===currentUser.id;
