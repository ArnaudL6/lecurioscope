...
export async function submitMysteryGuess(mysteryId,culpritAnswer){
  if(!state.currentUser){showToast('⚠️ Connecte-toi pour soumettre !');return;}
...
export async function submitMystery(ws){
  ...
  if(!suspect||reason.length<20){showSystemNotif({title:'R�ponse incompl�te  nom + raisonnement requis',xpGain:0});return;}
  ...
  if(ok){showSystemNotif({title:'Enqu�te r�solue�!',xpGain:50});if(typeof addXP==='function')addXP(50);}
