...
export async function buildLeagueDashboard(){
  if(!state.currentUser){
    document.getElementById('multi-content').innerHTML='<div style="text-align:center;padding:2rem;"><p style="color:var(--ink3);margin-bottom:1rem;">Connecte-toi pour acc�der � la ligue.</p><button class="btn-main" onclick="show(\'screen-login\')">Se connecter</button></div>';
    return;
  }
  document.getElementById('multi-content').innerHTML=
    ...
    '<button class="btn-sec" style="flex:1;margin-top:0;" onclick="goMultiPlay()"><� Priv�e</button>'+
    '</div>'+
    ...
    '<div style="margin-top:1rem;"><div class="sl-section-header" style="margin-bottom:.75rem;"><span class="sl-section-icon">=e</span><span>ALLI�S</span></div><div id="friend-list-own"></div></div>';
...
export async function goSoloPlay(){
  ...
  if(!state.currentUser){
    // Pas connect� � quiz du jour uniquement
  ...
  if(!anecIds.length){
    document.getElementById('multi-sub').textContent='';
    document.getElementById('multi-content').innerHTML='<div class="quiz-gate"><span class="quiz-gate-icon">=�</span><h3>Aucune anecdote lue</h3><p style="color:var(--ink3);font-size:.8rem;margin-top:.4rem;">Lis des anecdotes pour d�bloquer le quiz de r�vision !</p><button class="btn-main" style="margin-top:1rem;" onclick="goHome();updateNav(\'bn-anec\');">Lire maintenant</button></div>';
    return;
  }
...
export async function finishSoloHistoryQuiz(){
  ...
  const e=pct>=80?'<�':pct>=60?'P':'=�',t=pct>=80?'Excellent !':pct>=60?'Bien jou� !':'Continue !',m=pct>=80?'Parfaite ma�trise !':pct>=60?'Solide ! Reviens demain.':'Chaque jour on apprend.';
...
export function goMultiPlay(){if(typeof completeBingoCell==='function'){localStorage.setItem('bingo_multi','1');completeBingoCell(22);}
  ...
  document.getElementById('multi-sub').textContent='Cr�ez une salle ou rejoignez-en une avec un code.';
...
export function renderMultiLobbyChoice(){
  document.getElementById('multi-content').innerHTML=
    ...
    '<div class="multi-card" onclick="showCreateForm()"><span class="multi-card-icon"><�</span><div class="multi-card-title">Cr�er une salle</div><div class="multi-card-desc">H�bergez une partie et invitez vos amis avec un code � 4 chiffres.</div></div>'+
    '<div class="multi-card" onclick="showJoinForm()"><span class="multi-card-icon">=</span><div class="multi-card-title">Rejoindre</div><div class="multi-card-desc">Entrez le code donn� par l\'h�te pour rejoindre sa partie.</div></div>'+
    '</div>';
}
...
export function showCreateForm(){
  document.getElementById('multi-content').innerHTML=
    ...
    '<div style="font-size:.56rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:var(--a);margin-bottom:.2rem;">Partie priv�e</div>'+
    ...
    '</div>'+
    '</div>'+
    '<button class="btn-main" onclick="createRoom()">Cr�er la salle �</button>'+
...
export async function createRoom(){
  ...
  document.getElementById('multi-content').innerHTML=
    ...
    '<button onclick="navigator.clipboard.writeText(\''+code+'\').then(()=>showToast(\' Code copi� !\'))" style="margin-top:.65rem;padding:.32rem 1.1rem;border-radius:.5rem;border:1px solid var(--a);background:var(--adim);color:var(--a);font-family:inherit;font-size:.68rem;font-weight:700;letter-spacing:.09em;text-transform:uppercase;cursor:pointer;">=� Copier le code</button>'+
    '</div>'+
...
export async function joinRoom(){
  ...
  if(error||!session){showToast('� Salle introuvable ou partie d�j� commenc�e');return;}
  ...
  document.getElementById('multi-content').innerHTML=
    ...
    '</div>'+
    '</div>';
  document.getElementById('multi-sub').textContent="En attente de l'h�te&";
  state.multiChannel=sb.channel('room:'+code)
    .on('broadcast',{event:'game_start'},(p)=>startMultiClient(p.payload))
    .on('broadcast',{event:'player_ready'},(p)=>{state.multiState.readyPlayers=state.multiState.readyPlayers||new Set();state.multiState.readyPlayers.add(p.payload?.userId);checkMultiAllReady(p.payload?.qIdx);})
    .on('broadcast',{event:'game_end'},()=>showMultiScoreboard())
...
export async function finishLeague(){
  ...
  document.getElementById('multi-content').innerHTML=
    ...
    '</div>';
  // Mettre � jour le classement dans l'onglet social
