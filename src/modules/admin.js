...
export async function adminShowStats(el){
  ...
  el.innerHTML=
    ...
    '<div class="admin-stat-row"><span>Quiz jou�s</span><span class="admin-stat-val">'+(totalQuiz||0)+'</span></div>'+
    '<div class="admin-stat-row"><span>Duels termin�s</span><span class="admin-stat-val">'+(totalDuels||0)+'</span></div>'+
    '<div class="admin-stat-row"><span>Commentaires</span><span class="admin-stat-val">'+(totalComments||0)+'</span></div>'+
    '<div class="admin-stat-row"><span>Derni�re anecdote</span><span class="admin-stat-val">'+(recentAnec?.theme||'')+' ('+( recentAnec?.date||'')+')</span></div>'+
    '</div>'+
    (topUsers&&topUsers.length?
      '<div class="admin-section"><div class="admin-section-title"><� Top XP</div>'+
      topUsers.map((u,i)=>'<div class="admin-stat-row"><span>'+(i===0?'>G':i===1?'>H':i===2?'>I':'  #'+(i+1))+' '+u.username+'</span><span class="admin-stat-val">'+u.xp+' XP</span></div>').join('')+
      '</div>':''
    );
}
...
export async function adminShowAnecdotes(el){
  ...
  html+=`<div class="admin-section">
    ...
    <div style="font-size:.72rem;color:var(--ink2);margin-bottom:.65rem;">Supprime toutes les donn�es li�es � une anecdote (quiz, lectures, notes&) et revient au s�lecteur de th�me.</div>
...
export async function adminForceReset(mode){
  ...
  if(!confirm('Supprimer toutes les donn�es pour le '+date+' ?'))return;
  ...
  if(id){
    ...
    showToast('� Aucune anecdote trouv�e pour le '+date);return;
  }

  showToast(' Anecdote du '+date+' supprim�e !');
...
export async function adminDeleteAnecById(id, date, btn){
  if(!confirm('Supprimer l\'anecdote du '+date+' et toutes ses donn�es ?'))return;
  btn.disabled=true; btn.textContent='&';
  const res=await runAdminCascadeDelete(id);
  if(res.error==='no_key'){btn.textContent='� Cl�';btn.disabled=false;adminSwitchTab('anecdotes',null);return;}
...
export async function adminResetDay(){
  if(!isAdmin())return;
  if(!confirm('Supprimer l\'anecdote du jour et g�n�rer une nouvelle ?'))return;
...
export async function adminShowUsers(el){
  ...
  el.innerHTML='<div style="margin-bottom:.75rem;"><input id="admin-user-search" type="text" placeholder="=  Rechercher un utilisateur&" style="width:100%;padding:.55rem .85rem;background:var(--s1);border:1px solid var(--b2);border-radius:.6rem;font-family:sans-serif;font-size:.75rem;color:var(--ink);outline:none;box-sizing:border-box;" oninput="adminFilterUsers(this.value)" /></div><div id="admin-users-list"></div>';
  adminFilterUsers('');
}
...
export function adminFilterUsers(q){
  ...
  if(!filtered.length){list.innerHTML='<div class="admin-section"><div style="color:var(--ink3);font-size:.75rem;text-align:center;padding:.5rem 0;">Aucun r�sultat pour "'+q+'"</div></div>';return;}
  const admins=filtered.filter(u=>u.role==='admin');
  const mods=filtered.filter(u=>u.role==='moderator');
  const others=filtered.filter(u=>u.role!=='moderator'&&u.role!=='admin');
  let out='';
  if(term){
    out+=`<div class="admin-section"><div class="admin-section-title">${filtered.length} r�sultat${filtered.length>1?'s':''}</div>${filtered.map(userRow).join('')}</div>`;
  }else{
    if(admins.length)out+=`<div class="admin-section"><div class="admin-section-title">� Admins</div>${admins.map(userRow).join('')}</div>`;
    if(mods.length)out+=`<div class="admin-section"><div class="admin-section-title">=� Mod�rateurs (${mods.length})</div>${mods.map(userRow).join('')}</div>`;
    out+=`<div class="admin-section"><div class="admin-section-title">=e Membres (${others.length})</div>${others.map(userRow).join('')}</div>`;
  }
  list.innerHTML=out;
}
...
export async function adminToggleMod(uid,currentRole,btn){
  ...
  showToast(newRole==='moderator'?' Mod�rateur nomm� !':' R�le retir�');
  adminShowUsers();
}
