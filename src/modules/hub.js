...
const BINGO_CELLS=[
  ...
  {id:22,e:'<�', t:"Joue une partie priv�e"},
...
export async function buildWeeklyLeague(el){
  ...
  // R�cup�rer aussi le score du joueur si pas dans le top 20
  let myEntry=null;
  if(!myInTop){
  ...
  if(!top20.length){
    ...
    if(!myInTop&&myEntry){
      ...
      html+='<div style="border-top:1px dashed var(--b2);margin:8px 0;padding-top:8px;font-size:.65rem;color:var(--ink3);text-align:center">Vous n\'�tes pas encore class� cette semaine</div>';
    }
  }
  el.innerHTML=html;
}
...
export async function buildActivityFeed(el){
  ...
  if(!friendIds.length){el.innerHTML='<div class="league-section-title">=� Activit� des amis</div><div class="empty"><span class="empty-ico">=e</span><p>Ajoutez des amis pour voir leur activit�.</p></div>';return;}
  ...
  if(!events.length){el.innerHTML='<div class="league-section-title">=� Activit� des amis</div><div class="empty"><span class="empty-ico">=�</span><p>Aucune activit� r�cente.</p></div>';return;}
  el.innerHTML='<div class="league-section-title">=� Activit� des amis</div>'+
...
export async function answerChallenge(challengeId,answer,correct_answer){
  if(!state.currentUser){showToast('� Connecte-toi pour jouer !');return;}
  ...
  if(correct)showToast('<� Bonne r�ponse !');else showToast('L Rat� ! Retente la semaine prochaine.');
  // Bingo: marquer "d�fi communautaire fait"
  completeBingoCell(14);
  // Recharger le challenge pour afficher r�sultats
...
export function updateBingoFab(){
  ...
  if(ptxt)ptxt.textContent=n+' / 25 cases coch�es';
}
...
export async function buildCommunityChallenge(el){
  ...
  if(!ch){el.innerHTML='<div class="empty"><span class="empty-ico"><�</span><p style="color:var(--ink3);font-size:.8rem">D�fi de la semaine bient�t disponible !</p></div>';return;}
  ...
  if(answered){
    ...
    bottomHtml='<div class="challenge-result"><span>'+(ok?'':'L')+'</span><span>'+(ok?'Bonne r�ponse ! Bien jou� <�':'Rat� ! La bonne r�ponse est <strong>'+opts[ch.answer]+'</strong>')+'</span></div>'+
      ...
      '<div class="challenge-stats">'+nbOk+' / '+total+' joueurs ont trouv� ('+pctOk+'%)</div>';
  ...
  el.innerHTML='<div class="challenge-card">'+
    '<div class="challenge-week">'+ch.icon+' D�fi de la semaine</div>'+
...
export async function showHub(){
  ...
  const gateEnigma={rank:'B',icon:'=',name:'Crack le code !',desc:"R�sous l'�nigme du jour",xp:'+150 XP',done:enigmaToday,action:"goEnigme()"};
  ...
  function gateHTML(g){
    ...
    return `<div class="sl-gate ${g.done?'sl-gate-done':''}" onclick="${g.action}" style="--gate-color:${rk.color};--gate-glow:${rk.glow};">
      ...
      <div class="sl-gate-action">
        ${g.done
          ? '<span class="sl-gate-done-badge"> COMPL�T�</span>'
  ...
  <div class="sl-hub-cols">
    <div class="sl-hub-left">
      ...
      <div class="sl-hunter-card" style="--rank-color:${rank.color};--rank-glow:${rank.glow};--rank-bg:${rank.bg};">
        ...
        <div class="sl-xp-section">
          <div class="sl-xp-row">
            ...
            ${nextRank?`<span class="sl-xp-next">/ ${nextRank.minXP.toLocaleString('fr-FR')} � ${nextRank.label}</span>`:'<span class="sl-xp-next">P RANG MAXIMUM</span>'}
          </div>
        ...
        <div class="sl-hunter-streak">
          ...
          ${done===3?'<span class="sl-missions-done">� Missions compl�tes !</span>':'<span class="sl-missions-left">'+(3-done)+' mission'+(3-done>1?'s':'')+' restante'+(3-done>1?'s':'')+'</span>'}
        </div>
        <button class="sl-stats-btn" onclick="showStatsWindow()">=� Stats</button>
      </div>`:`
      <div class="sl-guest-card">
        <div class="sl-system-label">� THE SYSTEM</div>
        <div style="font-size:.85rem;color:var(--ink3);margin-top:.5rem;">Connecte-toi pour rejoindre le Syst�me.</div>
        <button class="sl-btn-primary" style="margin-top:1rem;" onclick="show('screen-login')">S'�veiller �</button>
      </div>`}
      ...
      <div class="sl-arena-section">
        <div class="sl-arena-gate" onclick="show1vs100Lobby()">
          ...
          <div class="sl-arena-enter">� ENTRER DANS L'AR�NE</div>
        </div>
      </div>
    ...
    <div class="sl-hub-right">
      ...
      <div class="sl-section-header sl-section-locked">
        <span class="sl-section-icon">=</span>
        <span>PORTAILS FERM�S</span>
        <span class="sl-section-count">BIENT�T</span>
      </div>
      <div class="sl-locked-gates">
        ${[
          {rank:'S',icon:'<�',name:'Mais dis moi ?'},
          {rank:'A',icon:'=�',name:'�ph�m�ride'},
