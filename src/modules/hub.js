...
export async function buildWeeklyLeague(el){
  if(!el)return;
  if(!state.currentUser){el.innerHTML='<div class="empty"><span class="empty-ico">ð</span><p>Connectez-vous pour voir le classement.</p></div>';return;}
  const now=new Date();
  ...
  function rowHtml(uid,rank,isMe){
    ...
    const statsHtml=(correct||wrong)?'<div style="font-size:.55rem;color:var(--ink3)">â'+correct+' â'+wrong+'</div>':'';
  ...
  let html='<div class="league-section-title">ð Classement semaine â Top 20</div>';
  if(!top20.length){
...
export async function buildActivityFeed(el){
  ...
  if(!events.length){el.innerHTML='<div class="league-section-title">ð£ ActivitÃ© des amis</div><div class="empty"><span class="empty-ico">ð«</span><p>Aucune activitÃ© rÃ©cente.</p></div>';return;}
  el.innerHTML='<div class="league-section-title">ð£ ActivitÃ© des amis</div>'+
...
export async function buildPendingRequests(el){
  ...
  el.innerHTML='<div class="pending-req-title">ð '+pending.length+' demande'+(pending.length>1?'s':'')+' d\'ami</div>'+
    pending.map(f=>{
      ...
      return '<div class="pending-req-item">'+
        ...
        '<div class="pending-req-btns">'+
          '<button class="btn-accept-req" onclick="acceptFriend(\''+f.id+'\')">â Accepter</button>'+
          '<button class="btn-decline-req" onclick="declineFriend(\''+f.id+'\')">â</button>'+
        '</div>'+
      '</div>';
    }).join('');
}
...
export async function showHub(){
  ...
  const gateAnec={rank:'E',icon:'ð¡',name:'Le Saviez-Vous ?',desc:"L'anecdote surprenante du jour",xp:'+50 XP',done:readToday,action:"goAnec()"};
  const gateEnigma={rank:'B',icon:'ð',name:'Crack le code !',desc:"RÃ©sous l'Ã©nigme du jour",xp:'+150 XP',done:enigmaToday,action:"goEnigme()"};
  const gateQuiz={rank:'C',icon:'ð¯',name:'Quiz',desc:"Teste tes connaissances",xp:'+100 XP',done:quizToday,action:"renderPlayChoice();show('screen-play');updateNav('bn-play');"};
  ...
  function gateHTML(g){
    ...
    return `<div class="sl-gate ${g.done?'sl-gate-done':''}" onclick="${g.action}" style="--gate-color:${rk.color};--gate-glow:${rk.glow};">
      ...
      <div class="sl-gate-action">
        ${g.done
          ? '<span class="sl-gate-done-badge">â COMPLÃTÃ</span>'
          : '<span class="sl-gate-enter">â¶ ENTRER</span>'}
      </div>
    </div>`;
  }
...
<div class="sl-hub">
  ...
  <div class="sl-system-header">
    <div class="sl-system-label">â¡ THE SYSTEM</div>
  ...
  <div class="sl-hub-cols">
      ...
      <div class="sl-hunter-card" style="--rank-color:${rank.color};--rank-glow:${rank.glow};--rank-bg:${rank.bg};">
        ...
        <div class="sl-xp-section">
          <div class="sl-xp-row">
            ...
            ${nextRank?`<span class="sl-xp-next">/ ${nextRank.minXP.toLocaleString('fr-FR')} â ${nextRank.label}</span>`:'<span class="sl-xp-next">â­ RANG MAXIMUM</span>'}
          </div>
        ...
        <div class="sl-hunter-streak">
          <span class="sl-streak-icon">ð¥</span>
          <span>${state.userStreak} jour${state.userStreak!==1?'s':''} de streak</span>
          ${done===3?'<span class="sl-missions-done">â¡ Missions complÃ¨tes !</span>':'<span class="sl-missions-left">'+(3-done)+' mission'+(3-done>1?'s':'')+' restante'+(3-done>1?'s':'')+'</span>'}
        </div>
        <button class="sl-stats-btn" onclick="showStatsWindow()">ð Stats</button>
      </div>`:`
      <div class="sl-guest-card">
        <div class="sl-system-label">â¡ THE SYSTEM</div>
        <div style="font-size:.85rem;color:var(--ink3);margin-top:.5rem;">Connecte-toi pour rejoindre le SystÃ¨me.</div>
        <button class="sl-btn-primary" style="margin-top:1rem;" onclick="show('screen-login')">S'Ã©veiller â</button>
      </div>`}

      <div class="sl-section-header sl-section-arena">
        <span class="sl-section-icon">â</span>
        <span>ARÃNE</span>
    ...
    <div class="sl-hub-right">
      ...
      <div class="sl-section-header">
        <span class="sl-section-icon">â</span>
        <span>QUÃTES JOURNALIÃRES</span>
      ...
      <div class="sl-section-header sl-section-locked">
        <span class="sl-section-icon">ð</span>
        <span>PORTAILS FERMÃS</span>
        <span class="sl-section-count">BIENTÃT</span>
      </div>
      <div class="sl-locked-gates">
        ${[
          {rank:'S',icon:'ð',name:'Mais dis moi ?'},
          {rank:'A',icon:'ð',name:'ÃphÃ©mÃ©ride'},
          {rank:'S',icon:'ð¬',name:"T'as dit quoi ?!"},
        ].map(g=>{
